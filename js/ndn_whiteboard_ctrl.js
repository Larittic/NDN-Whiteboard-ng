ndnWhiteboardApp.controller('ndnWhiteboardCtrl',
  function ($scope, $window, $exceptionHandler, util, ndn, Group, config) {
    // DEBUG
    ...
    $scope.submitSetting = function () {
      ...
      $window.onbeforeunload = function (event) {
        ...
        // dialog. Starting with Firefox 44, Chrome 51, Opera 38 and Safari 9.1,
    ...
    const joinGroup = function () {
      ...
      // Callback to handle received data. Note that all callbacks that
      // manipulate $scope data should be wrapped in $scope.$apply() for them to
      // be updated timely.
    ...
    // Leaves the current group by removing registered prefixes and notifying
    // the manager. If the user is group manager, instead of notifying the
    // manger, it will send manager_leave interest to another member if there is
    // any. The member become the new manager and notify the group.
    //
